window.onload = function () {
    var dom = document.getElementById("clock"),
        clk = dom.getContext('2d'),//获取绘图区域
        width = clk.canvas.width,
        height = clk.canvas.height,
        r  = width / 2,
        rem = width / 400;  //定义一个变量，存放一个粗细比例，以便于改变画布大小时可以智能调节全局粗细大小
    ...
    function drawBackground() {
        ...
        hourNumbers.forEach(function (number, i) {
            var rad = 2*Math.PI/12*i, //数字对应的弧度
                x = Math.cos(rad)*(r-30*rem),  //数字的x轴
                y = Math.sin(rad)*(r-30*rem);  //数字的y轴
            clk.fillText(number,x,y);//这里要注意：如何填充文字
        })

        for ( i = 0; i <= 60; i++) {
            var rad = 2*Math.PI/60*i,
                x = Math.cos(rad)*(r - 16*rem),
                y = Math.sin(rad)*(r - 16*rem);
            clk.beginPath();
            if (i%5 === 0) {
